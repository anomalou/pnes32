...
extern "C" void controller_init()
{
  ...
  pinMode(HW_CONTROLLER_GPIO_MENU, INPUT_PULLUP);
...
extern "C" uint32_t controller_read_input()
{
  ...
  m = digitalRead(HW_CONTROLLER_GPIO_MENU);
  // y = digitalRead(HW_CONTROLLER_GPIO_Y);

  return 0xFFFFFFFF ^ ((!u << 0) | (!d << 1) | (!l << 2) | (!r << 3) | (!s << 4) | (!t << 5) | (!a << 6) | (!b << 7) | (!m << 8));
}
